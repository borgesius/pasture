/**
 * The cow's moo: a real recording, pitched by breed size so a Dexter squeaks
 * and a Chianina rumbles. Falls back to a synthesised moo when the recording
 * cannot play. Resolves when done.
 */
export function moo(size = 1): Promise<void> {
  if (typeof Audio === "undefined") return synthMoo(size)
  return new Promise((resolve) => {
    const audio = new Audio("/sounds/moo-01.mp3")
    ...
    audio.onerror = () => synthMoo(size).then(finish, finish)
    audio.play().catch(() => synthMoo(size).then(finish, finish))
  })
}
